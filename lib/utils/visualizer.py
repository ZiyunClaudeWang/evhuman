...
def draw_kpts(img, kpts, r=5, thickness=5, color=(255,0,0), confidence=1e-6, draw_id=True):
    ...
    for kpid, kpt in enumerate(kpts):
        if len(kpt)>2:
            x, y, c = kpt
        else:
            x, y = kpt
        ...
        if draw_id:
            cv2.putText(img, str(kpid), (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, .5, color, 1, cv2.LINE_AA)
...
def __save_ply(vert, face=None, filename='file.ply'):
    # Vertices
    if isinstance(vert, np.ndarray):
        vert = vert.tolist()
    vert = [tuple(v) for v in vert]
