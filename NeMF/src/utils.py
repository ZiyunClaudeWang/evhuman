...
def compute_trajectory(velocity, up, origin, dt, up_axis='z', no_height=False):
    """
    Args:
        velocity: (B, T, 3)
        up: (B, T)
    ...
    if no_height:
        v_axis = [x for x in ordermap.values()]
    else:
    ...
    if no_height:
        for t in range(1, up.shape[1]):
            ...
            trajectory[:, t, v_axis[2]] = trajectory[:, t - 1, v_axis[2]] + velocity[:, t - 1, v_axis[2]] * dt
    else:
        for t in range(1, up.shape[1]):
            trajectory[:, t, v_axis[0]] = trajectory[:, t - 1, v_axis[0]] + velocity[:, t - 1, v_axis[0]] * dt
            trajectory[:, t, v_axis[1]] = trajectory[:, t - 1, v_axis[1]] + velocity[:, t - 1, v_axis[1]] * dt
