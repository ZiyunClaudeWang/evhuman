...
class ForwardKinematicsLayer(nn.Module):
    ...
    def __init__(self, args=None, parents=None, positions=None, device=None, smpl_path=None):
        super().__init__()
        self.b_idxs = None
        if device is None:
        ...
        if parents is None and positions is None:
            ...
            if smpl_path is not None:
                smpl_fname = smpl_path
                import pickle as pkl
                # smpl_data = np.load(smpl_fname, encoding='latin1')
                smpl_data = pkl.load(open(smpl_fname, 'rb'), encoding='latin1')
            else:
