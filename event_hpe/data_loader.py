...
class TrackingDataloader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # frame_idx = 1138
        if self.mode == 'train' and not self.no_aug:
        ...
        for i in range(self.num_steps):
            ...
            for j in range(start_idx, end_idx):
                ...
                if self.use_volumes:
                    volume_dir = os.path.join(os.path.dirname(self.data_dir), "mmhpsd_volumes")
                    single_events_frame.append(cv2.imread(
                        '%s/%s/events_%04i.png' % (volume_dir, action, j), -1))
                else:
                    single_events_frame.append(cv2.imread(
                        '%s/events_%i/%s/event%04i.png' % (self.data_dir, self.img_size, action, j), -1))

                if self.raw_events:
                    event_file = '%s/%s/events/events_%04i.npz' % (event_folder, action, j)
                    event_file = np.load(event_file)
                    event_array = np.concatenate([event_file['xy'], event_file['t'][:, None], event_file['p'][:, None]], axis=1)
                    event_array[:, 2] = (event_array[:, 2] - event_array[0, 2]) / (event_array[-1, 2] - event_array[0, 2]) * 0.8 + current_time
                    single_raw_events.append(event_array)
                    current_time += 1

            single_events_frame = np.concatenate(single_events_frame, axis=2).astype(np.float32)  # [H, W, C]
            if self.raw_events:
                single_raw_events = np.concatenate(single_raw_events, axis=0)
            ...
            raw_events.append(single_raw_events)
            ...
            frame = cv2.imread('%s/full_pic_%i/%s/fullpic%04i.jpg' % (self.data_dir, self.img_size, action, end_idx), cv2.IMREAD_GRAYSCALE)
        ...
        # frames = np.stack(frame,s axis=0)
        ...
        if self.raw_events:
            events_all = np.concatenate(raw_events, axis=0)
            event_breaks = np.cumsum([len(e) for e in raw_events])
        ...
        one_sample['intri'] = torch.from_numpy(self.cam_intr).float()  # [4]

        if self.raw_events:
            one_sample['raw_events'] = torch.from_numpy(events_all).float()  # [N, 4]
            one_sample['event_breaks'] = event_breaks
            one_sample['event_size'] = events_all.shape[0]

        frames = np.stack(frames, axis=0)
        one_sample['frames'] = torch.from_numpy(frames).float() # [T, H, W]
